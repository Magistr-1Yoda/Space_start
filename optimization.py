...
bot = telebot.TeleBot('6215262539:AAHL0rfnlAqweqNpPyIPIcmfwSLx99mxkdA')
...
def welcome_message(message):
    ...
    if score[1] == 0:
        bot.send_message(message.from_user.id,f'○★~Привет из космоса для всех любителей необъятной вселенной!!! Жаждите новой информации? Хотите проверить свой уровень знаний? Тогда, не будем томить : "поехали!"~★○', reply_markup=keyboards_create([ "📚Викторина!" , '👀 Факт о космосе!', '👨‍🚀 Цитаты космонавтов!' , '🖼Постеры про космос!', '🔥Мультисериал']))
        db.visit(id)
        bot.register_next_step_handler(message, check_choice)
    else:
        bot.send_message(message.from_user.id,f'Вы вернулись в главное меню, здесь вы можете посмотреть другие разделы🔎', reply_markup=keyboards_create([ "📚Викторина!" , '👀 Факт о космосе!', '👨‍🚀 Цитаты космонавтов!' , '🖼Постеры про космос!', '🔥Мультисериал']))
        bot.register_next_step_handler(message, check_choice)
...
def check_choice(message):
    choice = message.text
    list_check = ['1 серия🎬🍿','2 серия🎬🍿','3 серия🎬🍿','4 серия🎬🍿', 
        '1 Постер','2 Постер','3 Постер','4 Постер','5 Постер','6 Постер',
        '🔎 Цитата Юрия Гагарина','🔎 Цитата Глушко Валентина','🔎 Цитата Джанибекова Владимира','🔎 Цитата Королёва Сергея','🔎 Цитата Криколёва Сергея',
        '🔎 Цитата Леонова Алексея','🔎 Цитата Севастьянова Виталия','🔎 Цитата Терешковой Валентины','🔎 Цитата Титова Германа','🔎 Цитата Уточкина Сергея',
        '🔎 Цитата Фёдорова Николая','🔎 Цитата Цилковского Константина','🔎 Цитата Челомия Владимира','🔎 Цитата Чертока Бориса','🔎 Цитата Юрчихина Фёдора',]
    ...
    if choice == "📚Викторина!":
        ...
        choice_quote(message, list_check, list_quote_back)

    elif choice == '🔥Мультисериал':
        list_series_back = ['🔙 Главное меню','1 серия🎬🍿','2 серия🎬🍿','3 серия🎬🍿','4 серия🎬🍿']
        choice_series(message, list_series_back, list_check)
...
def fact(message):
    for i in range(1):
        bot.send_message(message.from_user.id, random.choice(spacefacts), reply_markup=keyboards_create([ "📚Викторина!" , '👀 Факт о космосе!', '👨‍🚀 Цитаты космонавтов!' , '🖼Постеры про космос!', '🔥Мультисериал']))
        bot.register_next_step_handler(message, check_choice )
        

def choice_posters(message, list_check, list_posters_back):
    ...
    bot.register_next_step_handler(message, list_check, list_posters_back)

def check_poster(message, list_posters_back, list_check):
    posters = message.text
    if posters in list_check:
        link = db.start_posters(posters)
...
def choice_quote(message, list_check, list_quote_back):
    ...
    bot.register_next_step_handler(message, check_quote, list_quote_back, choice_quote)

def check_quote(message, list_quote_back, list_check):
    quote = message.text
    if quote in list_quote_back:
        link = db.start_quote(quote)
        ...
        bot.register_next_step_handler(message, check_quote, list_quote_back, list_check)
    else:
        welcome_message(message)

def choice_series(message, list_check, list_series_back):
    bot.send_message(message.from_user.id, '⬇ Выберите серию которую вы желаете посмотреть', reply_markup=keyboards_create_posters(list_check))
    bot.register_next_step_handler(message, check_series, list_series_back, list_check)

def check_series(message, list_series_back, list_check):
    series = message.text
    if series in list_series_back:
        link = db.start_series(series)
        bot.send_message(message.from_user.id, f'Вы выбрали {link[0]}\n \n👍🏻 Отличный выбор\n \n📲 Посмотреть эту серию можно по ссылке🌐👇🏻\n{link[1]}', reply_markup=keyboards_create(list_check))
        bot.register_next_step_handler(message, check_series, list_series_back, list_check)
    else:
        welcome_message(message)
